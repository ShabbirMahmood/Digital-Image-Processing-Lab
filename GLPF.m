function [ output_img2 ] = GLPF(input_img,D0)  % (Gaussian Low Pass Filter of Order n) Page-276
[M,N] = size(input_img);
...
GW_Low_filter = exp(-((u.^2+v.^2)./(2.*D0.^2))); % Equation of Butterworth D(u,v) = (u.^2+v.^2) for Non Padded Image
%GW_Low_filter =1.-exp(-((u.^2+v.^2)./(2.*D0.^2))); % High Pass

% Performing Filtering in Frequency Domain
output_img = f_img .* GW_Low_filter; % Convolution in Spatial Domain = Multiplication in Frequency Domain
